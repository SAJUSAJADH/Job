...
async function Home() {
  ...
  return (
    ...
    <section className="container relative mt-20 flex flex-col justify-center gap-2 items-center">
        <Image src='/apple.gif' alt='' width={50} height={50} className='hidden lg:block absolute top-0 left-36 -rotate-45' unoptimized/>
        <Image src='/spotify.gif' alt='' width={50} height={50} className='hidden lg:block absolute bottom-0 right-36' unoptimized/>
        <Image src='/twitter.gif' alt='' width={50} height={50} className='hidden lg:block absolute top-0 right-36 scale-x-[-1]' unoptimized/>
        <Image src='/netfix.gif' alt='' width={50} height={50} className='hidden lg:block absolute bottom-0 left-36' unoptimized/>
        <h1 className="text-3xl lg:text-7xl font-bold text-center">
          Find your remote job <br />
          <TypeAnime/>
        </h1>
        <p className="text-center text-sm lg:text-base text-gray-600 mt-2 px-4 lg:px-80">
        Build your best job community starting from here. One Stop Solution to Find Jobs.
        </p>
        <form action={handleSearch} className="flex gap-2 mt-4 max-w-md mx-auto">
          <input
            type="search"
            name="search"
